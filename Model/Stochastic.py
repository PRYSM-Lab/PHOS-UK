...
pccostWE_data=excel_data2.parse('pccost')
pocostFWE_data=excel_data2.parse('pocostF')
pccosVWE_data=excel_data2.parse('pocostV')
rccost_data=excel_data2.parse('rccost')

gasprice_data=excel_data2.parse('gasprice')
...
Biomass_data20 = excel_data2.parse('biomass')
#Demand_data = excel_data.parse('Demand6c2017')
etaWE_data=excel_data2.parse('eta')
...
df_cgas20 = excel_data2.parse('gasprice', header=None, usecols="A:U", skiprows=2, nrows=3)
...
#df_Availability = excel_data.parse('Sheet1', header=None, usecols="A:AO", skiprows=2, nrows=144 )
#df_GasDem = excel_data.parse('Demand6c2017', header=None, usecols="A:O", skiprows=1, nrows=144)
#df_GasDem = excel_data.parse('Demand6cNEW2017', header=None, usecols="A:O", skiprows=1, nrows=144)
df_ComDem20= excel_data2.parse('Commercial', header=None, usecols="A:JB", skiprows=3, nrows=120)
df_DomDem20= excel_data2.parse('Domestic', header=None, usecols="A:JB", skiprows=3, nrows=120)
df_IndDem20= excel_data2.parse('Industrial', header=None, usecols="A:JB", skiprows=3, nrows=120)
df_hdem20=excel_data2.parse('demand', header=None, usecols="A:V", skiprows=2, nrows=12)
df_Availability20 = excel_data2.parse('AV20', header=None, usecols="A:JC", skiprows=3, nrows=363)
df_rccost20=excel_data2.parse('rccost',header=None, usecols="A:V", skiprows=2, nrows=9 )
Prob_data=excel_data2.parse('prob')
# %% Building the Model===================================
...
model.k=Set(initialize=["k" + str(i) for i in range(1, 21)])
...
AV_data = {(c, h, e, g, k): df_Availability20.iloc[i, 3 +  20*g_idx+k_idx]
    for i, (c, h,e) in enumerate(zip(df_Availability20.iloc[:, 0], df_Availability20.iloc[:, 1], df_Availability20.iloc[:, 2]))  
    for g_idx, g in enumerate(model.g)  
...
Data_price=gasprice_data.iloc[1:4,1:21]
cgas_data20 = {(t,k): Data_price.iloc[i,j]
...
eta_dataWE=etaWE_data.iloc[1:4,2:22]
...
Dom_data={(c, h, g,k): df_DomDem20.iloc[i, 2 +  20*g_idx+k_idx] 
               for i, (c, h) in enumerate(zip(df_DomDem20.iloc[:, 0], df_DomDem20.iloc[:, 1]))  
               for g_idx, g in enumerate(model.g)  
               for k_idx, k in enumerate(model.k)}

Com_data={(c, h, g,k): df_ComDem20.iloc[i, 2 +  20*g_idx+k_idx] 
               for i, (c, h) in enumerate(zip(df_ComDem20.iloc[:, 0], df_ComDem20.iloc[:, 1]))  
               for g_idx, g in enumerate(model.g)  
               for k_idx, k in enumerate(model.k)}
Ind_data={(c, h, g,k): df_IndDem20.iloc[i, 2 +  20*g_idx+k_idx] 
               for i, (c, h) in enumerate(zip(df_IndDem20.iloc[:, 0], df_IndDem20.iloc[:, 1]))  
               for g_idx, g in enumerate(model.g)  
               for k_idx, k in enumerate(model.k)}

hdem_data={(o, t,k): df_hdem20.iloc[i, 2 +  k_idx] 
               for i, (o, t) in enumerate(zip(df_hdem20.iloc[:, 0], df_hdem20.iloc[:, 1]))   
               for k_idx, k in enumerate(model.k)}
...
DatapccostWE=pccostWE_data.iloc[1:4,1:21]
...
DatapocostWE = pocostFWE_data.iloc[1:4, 1:21]
...
DatapocostVWE = pccosVWE_data.iloc[1:4, 1:21]
...
rccost_data = {(e, t, k): df_rccost20.iloc[i, 2+k_idx]
    for i, (e, t) in enumerate(zip(df_rccost20.iloc[:, 0], df_rccost20.iloc[:, 1]))    
    for k_idx, k in enumerate(model.k)}

Data_prob = Prob_data.iloc[1:21,1]
...
Data25 = Biomass_data20.iloc[1:4, 1:21]
...
model.cgas = Param(model.TT,model.k, initialize=cgas_data20, doc='Natural gas cost in time period t (€/MWh)')
...
def dem_init(model, g, t, CC, h,k):
    return (model.hdem['Domestic',t,k] * model.Dom[CC, h, g,k]/288.6+model.hdem['Commercial',t,k] * model.Com[CC, h, g,k]/94.5+ 
            model.hdem['Industrial',t,k] * model.Ind[CC, h, g,k]/89.4+(model.hdem['Transportation',t,k] * 1000000/8760/len(model.g)) )
model.dem = Param(model.g, model.TT, model.CC, model.h, model.k, initialize=dem_init)
...
for p in model.p:
    for t in model.TT:
        for k in model.k:
            if model.ord_p[p] < 4:
                pccostK_data[(p, t, k)] = 2.5*model.pccost[p, t]
            elif model.ord_p[p] == 4:
                pccostK_data[(p, t, k)] = 2.5*model.pccostWE[t, k] / 1000
...
global n_slice  # استفاده از متغیر سراسری
# n_slice = None  # این خط نیازی نیست

def run_intraday(n_slice_input):
    print(f"Running scenario.py with time-slice: {n_slice_input}")

    # اجرای script و ارسال مقدار n_slice به آن
    result = subprocess.run(
        ['python', 'scenario.py', str(n_slice_input)],  # مقدار را مستقیم ارسال می‌کنیم
        capture_output=True,
        text=True
    )
...
# تابع برای دریافت مقدار از کاربر
def get_time_slice():
    global n_slice  # اطمینان از تغییر مقدار متغیر سراسری
    root = tk.Tk()
    root.withdraw()  
    root.geometry("800x400")
    n_slice = simpledialog.askinteger("Input", "Enter number of time-slices:\n(Max value: 24)")
    
    print(f"User entered n_slice: {n_slice}")  # برای دیباگ

    if n_slice is None:
        print("Error: No value entered for n_slice!")
        return  

    # اجرای `scenario.py` در یک نخ جداگانه
...
df_theta=excel_data3.parse('Sheet1', header=None, usecols="A:C", skiprows=1, nrows=121)
...
model.Hsum = Set(dimen=2, initialize=[(h, c) for (h, c) in Hsum_data.keys()])
...
def demH_init(model, g, t, c, HH, k):
    return sum(
        ...
        for h in model.HH if ((HH, c) in model.Hsum)
    )
...
def imp_limit_rule(model, t, c, h,k):
    if t in model.TT and c in model.CC and h in model.h and (c,h) in model.CH:
        return sum(model.IMP[g, t, c, h,k] for g in model.Gimp) <= model.iota*sum(model.dem[g, t, c, h,k] for g in model.g)
    return Constraint.Skip
