...
def First_step():
    ...
    #Data26 = Demand_data.iloc[0:4, 56]
    ...
    model.it = Set(initialize=[f'it{i}' for i in range(1, 9)])  # Equivalent to /it1*it8/
    model.iter_fesi = Set(initialize=model.it)  # Assuming iter_fesi is the same as iter(it)

    # Defining Stage2_it set
    model.Stage2_it = Set(initialize=[f'Stage2_it{i}' for i in range(1, 31)])  # Equivalent to /Stage2_it1*Stage2_it30/
    ...
    def s_availability_rule(model, s, g, t):
        if (g, s) in model.GS and t in model.TT:
            if t > model.y1:
                return model.NS[(s, g), t] == model.NS[s, g, t-1] + model.InvS[(s, g), t]
            elif t == model.y1:
                return model.NS[(s, g), t] == model.ns0[s, g] + model.InvS[(s, g), t]
        return Constraint.Skip

    model.SAvailabilityConstraint = Constraint(model.s,model.g, model.TT, rule=s_availability_rule)
    ...
    def renew_cap_rule(model, e, g, t):
        if t in model.TT:
            if t > 3:  # Check for t > y1 (y1 = 1 in this case)
                return model.NR[e, g, t] == model.NR[e, g, t - 1] + model.InvR[e, g, t]
            elif t==3:  
                return model.NR[e, g, t] == model.InvR[e, g, t]
        return Constraint.Skip
    ...
    def h2MassBalance_plus_rule(model, g, t, c, h):
        if model.ord_c[c] > 1:
            return (
            sum(model.Pr[p, g, t, c, h] for p in model.p)
            + sum(model.Q['Pipe', g1, g, t, c, h] for g1 in model.g if (g1, g) in model.Npipe)
            ...
            + sum(model.Qr[s, g, t, c, h] for s in model.s if (g, s) in model.GS)
            ==
            sum(model.Q['Pipe', g, g1, t, c, h] for g1 in model.g if (g1, g) in model.Npipe)
            + sum(model.Qi[g, s, t, c, h] for s in model.s if (g, s) in model.GS)
            + (model.GasDemave[c, h, t, g])
            + sum(model.lemma_neg[t, g, c, h, j] + model.lemma_pos[t, g, c, h, j] + model.gamma[t, g, c, h, j] for j in model.j)
            + model.xi[t, g, c, h] * model.phi)
        else:
            return Constraint.Skip

    model.MassBalance_plus = Constraint(model.g, model.TT, model.CC, model.HH, rule=h2MassBalance_plus_rule)
    ...
    def res_inventory_rule(model, r, t):
        if t in model.TT and t >= model.y1:  
            if t == 3:  
                return model.RI[r, t] == model.ri0[r]  / 1000+ model.dur * sum(
                    model.WF[c] * model.theta * model.Qoff[(g, r), t, c, h] for g in model.g
                   if (g, r) in model.GR for c in model.CC for h in model.h
                ) / 1000
            elif t > 3:
                return model.RI[r, t] == model.RI[r, t - 1] + model.ri0[r] / 1000+ model.dur * sum(
    ...
    def imp_limit_rule(model, t, c, h):
        if t in model.TT and c in model.CC and h in model.HH:
            return sum(model.IMP[g, t, c, h] for g in model.Gimp) <= 0.1*sum(model.GasDemave[c,h, t, g] for g in model.g)
        return Constraint.Skip
    model.ImpLimitConstraint = Constraint(model.TT, model.CC, model.HH, rule=imp_limit_rule)
    
    
    # %% Robust Constraint
    def dualinequ1_rule(model, t, g, c, h, j):
        if model.ord_c[c]>1:
            return model.lemma_neg[t, g, c, h, j] + model.gamma[t, g, c, h, j] + model.xi[t, g, c, h] >= model.BigQ_neg[c, h, j, t, g]
        else:
            return Constraint.Skip
    model.dualinequ1 = Constraint(model.TT, model.g, model.CC, model.h, model.j, rule=dualinequ1_rule)


    def dualinequ2_rule(model, t, g, c, h, j):
        if model.ord_c[c]>1:
            return model.lemma_pos[t, g, c, h, j] + model.gamma[t, g, c, h, j] + model.xi[t, g, c, h] >= model.BigQ_pos[c, h, j, t, g]
        else:
            return Constraint.Skip
    model.dualinequ2 = Constraint(model.TT, model.g, model.CC, model.h, model.j, rule=dualinequ2_rule)
    ...
    def h2pipe_max_rule(model, g, g1, t, c, h):
        if (g, g1) in model.Npipe and t in model.TT and c in model.CC and h in model.HH:
    ...
    '''
    def h2p_availability_rule(model, d1, g, g1, t):
        if (g, g1) in model.Npipe and model.ord_g[g] < model.ord_g[g1] and t in model.TT and  model.ord_d1[d1]==3:
            return model.AY[d1, g, g1, t] == (
                model.AY[d1, g, g1, t - 1] if t > model.y1 else 0
            ) + (
                model.ayHR0[d1, g, g1] if t == model.y1 else 0
            ) + model.Yh[d1, g, g1, t]
        return Constraint.Skip

    model.H2PAvailability = Constraint(model.d1, model.Npipe, model.TT, rule=h2p_availability_rule)

    def onp_availability_rule_simple(model, d2, g, g1, t):
        if (g, g1) in model.N and model.ord_g[g] < model.ord_g[g1] and t in model.TT and model.ord_d2[d2]==2:
           return model.AYon[d2, g, g1, t] == (
               model.AYon[d2, g, g1, t - 1] if t > model.y1 else 0
           ) + (
               model.ayC0[d2, g, g1] if t == model.y1 else 0
           ) + model.Yon[d2, g, g1, t]
        return Constraint.Skip
    ...
    def offp_availability_rule(model, d2, g, r, t):
         if (g, r) in model.GR and t in model.TT and model.ord_d2[d2]==2: 
            return model.AYoff[d2, (g, r), t] == (model.AYoff[d2, (g, r), t-1] if t>model.y1 else 0
            )+ (
            model.aeC0[r] if t==model.y1 else 0
            )+ model.Yoff[d2, (g, r), t]
         return Constraint.Skip
    ...
    def pipest_availability_rule(model, d1, g, sc, t):
          if (g, sc) in model.GS2 and t in model.TT and model.ord_d1[d1]==3: 
               return model.AYst[d1, (g, sc), t] == (model.AYst[d1, g, sc, t-1] if t> model.y1 else 0 
              )+ (model.Yst[d1, (g, sc), t])
          return Constraint.Skip
    ...
    df.to_excel("resultsHA2_1.xlsx", index=False, sheet_name="All Data")

    print("All variables and objective saved in 'resultsHA2_1.xlsx'")
...
# %% Define Variables------------------------------
    ...
    def offshorepipe_max_rule(model, g, r, t, c, h):
        if (g, r) in model.GR:
            return 0.001*model.Qoff[(g, r), t, c, h]  <= 0.001*sum(model.qCmax[d2] * model.AYoff[d2, (g, r), t] for d2 in model.d2 if model.ord_d2[d2]==2)
        return Constraint.Skip
    model.OffshorePipeMax = Constraint(model.GR, model.TT, model.CC, model.h, rule=offshorepipe_max_rule)
    ...
    df.to_excel("resultsHA2_2.xlsx", index=False, sheet_name="All Data")

    print("All variables and objective saved in 'resultsHA2_2.xlsx'")
